...
QTermWidget::QTermWidget(QWidget *parent,ConnectInfo* conInfo):
    QTermScreen(parent)
{
    this->connectInfo=*conInfo;
    m_Mode = 0;
    m_Echo = false;
    m_SLine = false;
    m_Cnt = 0;
    ...
    resize(this->width(),this->height()-STATUS_BAR_HIGHT);
...
//    if (byte.size() != 0)
//    {
//        sshClient->exec(byte.toStdString());
//    }

    sshClient->exec(e->text().toStdString());
}
...
{
    if(connectSuccess){
        sshClient->pty_cols=this->width()/4;
        int rows = (this->height()-STATUS_BAR_HIGHT-100)/4;
        qDebug() << rows;
        sshClient->pty_rows=rows;
    }
}

void QTermWidget::readChannelData(const char data){
    recvChar(data);
}

void QTermWidget::ssh2connect(){
